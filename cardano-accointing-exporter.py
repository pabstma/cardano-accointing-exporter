...
def main():
    ...
    parser.add_argument('--project-id-file', type=str, default='project.id',
                        help='path to a file containing only your blockfrost.io project id (has precedence over hard-coded project id in ' +
                             'config.py, default: project.id)')
    ...
    if args.project_id_file is not None:
        try:
            with open(args.project_id_file, 'r') as f:
                ...
                print('Project id file "' + args.project_id_file + '" read successfully ', end='')
                print(u'\u2713')
        except OSError as e:
    ...
    for wallet_file in config.wallet_files:
        ...
        config.address_counter = 0
        config.calculated_address_counter = 0
        config.tx_counter = 0
        ...
        for address in current_wallet.addresses:
            ...
            if current_wallet.stake_address.startswith('stake1u'):
                if current_wallet.stake_address not in stake_keys_calculated:
                    ...
                    current_wallet.rewards = koios.get_reward_history_for_account(current_wallet.stake_address, args.start_time,
                                                                                  args.end_time)
            ...
            print(LINE_CLEAR + '---- Received ' + str(config.tx_counter) + ' TXs - Elapsed Time: ' +
                  str(round(config.elapsed_time, 2)))
